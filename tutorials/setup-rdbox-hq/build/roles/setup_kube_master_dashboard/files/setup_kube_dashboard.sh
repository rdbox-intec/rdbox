...
kubectl -n kube-system create secret generic kubernetes-dashboard-certs --from-file=/home/${SUDO_USER}/.kube/dashboard/certs/

# restart kubernetes-dashboard with new certs by delete pods
while :
do
    KUBE_DASHBOARD_NAME=`kubectl -n kube-system describe pods kubernetes-dashboard | grep -e '^Name:' | sed -e 's#\s##g' | cut -d':' -f 2`
